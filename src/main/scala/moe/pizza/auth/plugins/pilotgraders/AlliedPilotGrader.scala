...
object AlliedPilotGrader {
  case class SavedContactList(cachedUntil: DateTime,
                              pilots: Seq[String],
                              corporations: Seq[String],
                              alliances: Seq[String])

  val CORPORATION = 2
  val ALLIANCE = 16159
  val CHARACTERS = Range(1373, 1386)

  def transformContacts(cachedUntil: DateTime,
                        contacts: Seq[corp.ContactList.Row]) = {
    val grouped = contacts.groupBy(_.contactTypeID)
    SavedContactList(
      cachedUntil,
      grouped
        .filterKeys(i => CHARACTERS.contains(i.toInt))
        .values
        .flatten
        .map(_.contactName)
        .toSeq,
...
class AlliedPilotGrader(threshold: Double,
                        usecorp: Boolean,
                        usealliance: Boolean,
  ...
  allies match {
    ...
    case None =>
      logger.info(
        s"loaded AlliedPilotGrader, but failed to load a configuration from CCP")
  }
  ...
  def pullAllies(): Option[SavedContactList] = {
    ...
    res match {
      case Success(r) =>
        val corp = r.result
          .filter(_.name == "corporateContactList")
          .flatMap(_.row)
          .filter(_ => usecorp)
          .filter(_.standing > threshold)
        val alli = r.result
          .filter(_.name == "allianceContactList")
          .flatMap(_.row)
          .filter(_ => usealliance)
          .filter(_.standing > threshold)
